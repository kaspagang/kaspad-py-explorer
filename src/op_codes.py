...
#Source: https://github.com/kaspanet/kaspad/blob/master/domain/consensus/utils/txscript/OP_code.go
OP_CODE_TO_BYTE = {
  #"OP_0"                             :               0x00 // 0
	...
	"OP_MAX"                           :               0xa4, # 164
	"OP_WITHIN"                        :               0xa5, # 165
	"OP_UNKNOWN166"                    :               0xa6, # 166
	"OP_UNKNOWN167"                    :               0xa7, # 167
	"OP_SHA256"                        :               0xa8, # 168
	"OP_CHECKMULTISIGECDSA"            :               0xa9, # 169
	"OP_BLAKE2B"                       :               0xaa, # 170
	"OP_CHECKSIGECDSA"                 :               0xab, # 171
	"OP_CHECKSIG"                      :               0xac, # 172
	"OP_CHECKSIGVERIFY"                :               0xad, # 173
	"OP_CHECKMULTISIG"                 :               0xae, # 174
	"OP_CHECKMULTISIGVERIFY"           :               0xaf, # 175
	"OP_CHECKLOCKTIMEVERIFY"           :               0xb0, # 176
	"OP_CHECKSEQUENCEVERIFY"           :               0xb1, # 177
	"OP_UNKNOWN178"                    :               0xb2, # 178
...
def decode_op_codes(script_pub_key):
        ...
        index = 0
        op_code_Parsed = ""
        for _ in range(len(op_code_bytes)):
                op_byte = op_code_bytes[index]
                op_code_Parsed = op_code_Parsed + BYTE_TO_OP_CODE[op_byte] + "\n" 
                if op_byte <= 78 and op_byte > 0:
                        if op_byte <= 75:
                                bytes_to_push = 0
                                data_size = op_byte
                        elif op_byte == 76:
                                bytes_to_push = 1
                                data_size = op_code_bytes[index + 1]
                        elif op_byte == 78:
                                bytes_to_push = 2
                                data_size = int.from_bytes(op_code_bytes[index + 1:index + 2], byteorder='big', signed=False)
                        elif op_byte == 79:
                                bytes_to_push = 4
                                data_size = int.from_bytes(op_code_bytes[index + 1:index + 4], byteorder='big', signed=False)
                        data = op_code_bytes[ index + 1 :index + 1 + bytes_to_push + data_size].hex()
                        op_code_Parsed = op_code_Parsed + f"{data_size} <{data}> \n"
                        index = index + 1 + data_size + bytes_to_push
                else:
                        index = index + 1
                if index == len(op_code_bytes):
                        break
        return op_code_Parsed
